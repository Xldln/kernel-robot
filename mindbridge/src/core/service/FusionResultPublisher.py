...
def require_zmq() -> Any:
    if zmq is None:
        raise RuntimeError("Fusion ZMQ publishing requires pyzmq.")
    return zmq
...
def build_siglip_payload(state_result: dict[str, Any] | None, frame_id: Any) -> dict[str, Any]:
    if not isinstance(state_result, dict):
        state_result = {}
    return {
        "frame_id": frame_id,
        "ok": bool(state_result.get("ok", state_result.get("status") == "ok")),
        "best_category": state_result.get("best_category"),
        "best_similarity": state_result.get("best_similarity"),
    }


class FusionResultPublisher:
    def __init__(
        self,
        addr: str = "tcp://0.0.0.0:8899",
        *,
        frame_id: str = "camera_rgb_link",
        tf_topic: str = "/tf",
    ...
    def publish(
        ...
        *,
        ...
        flowpose_result: dict[str, Any] | None,
    ) -> None:
