...
class RealsenseService:
    ...
    def _init_cameras(self, cfg: dict) -> None:
        if self.mode == "multi":
            ...
            # 选定 primary：role==primary 的第一台，否则列表第一台
            primary_entry = next(
    ...
    def capture_aux(self) -> dict[str, np.ndarray]:
        """采集全部 color-only 相机各一帧彩色图，返回 {cam_id: color_bgr}（按配置顺序）。"""
        return {cid: cam.capture_color() for cid, cam in self.aux_cameras.items()}
